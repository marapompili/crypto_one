...
ciphertexts = [unhexlify(c) for c in ciphertexts_hex]
target = unhexlify(target_hex)
...
for i, c1 in enumerate(ciphertexts):
    ...
    for j, c2 in enumerate(ciphertexts):
        ...
        for k, byte in enumerate(xored):
            if is_letter(byte):
                scores[k] += 1

    space_scores.append(scores)
...
for i, c in enumerate(ciphertexts):
    for k, score in enumerate(space_scores[i]):
        if score >= 8:   # soglia euristica
            key[k] = c[k] ^ ord(' ')
...
def decrypt_with_partial_key(ciphertext, key):
    ...
    for i, byte in enumerate(ciphertext):
        if i < len(key) and key[i] is not None:
            result.append(chr(byte ^ key[i]))
        else:
            result.append('*')

    return ''.join(result)

print(decrypt_with_partial_key(target, key))
